      ...
      http.request(request) do |response|
        response.read_body do |chunk|
          ...
          loop do
            ...
            # Break if remaining buffer size is less than message length,
            # (include extra space for linebreak and length header).
            break if (stream_buffer.size - msg_length_string.size - 2) <= msg_length 
            ...
            # Invoke the block with the message
            yield msg
          end
        end
      end
